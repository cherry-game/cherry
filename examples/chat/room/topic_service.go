...
func joinRoom(session *cherrySession.Session, req *protocol.JoinRoomRequest) error {
...
func newUser(s *cherrySession.Session, nickName string) error {
	...
	s.Push("onMembers", &protocol.ExistsMembersResponse{Members: strings.Join(members, ",")})
...
func stats(s *cherrySession.Session, uid int64) {
	...
	s.Push("onBalance", &protocol.UserBalanceResponse{CurrentBalance: user.balance})
